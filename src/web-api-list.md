# Rac.VOne.Web.Api list

ASP.NET Core MVC で作成した Web API の Controller / Method 一覧

パラメータや、検索用 Model の確認を行う


要否を判定してどうにかする

基本的に メソッド数が多すぎるのは問題なので 単一/複数 など 呼び出し側で如何様にでも対応できるものは、単一系を消す 無駄

他テーブルとjoin する系のものは、メソッドを追加して対応する必要はあるが、極力汎用的になるようにしたい。
...
|----------|----|-----------|---------|-------|
|AccountTitle|`IEnumerable<AccountTitle>`|Get|int[] ids|GetItems に集約可能|
|AccountTitle|`IEnumerable<AccountTitle>`|GetItems|AccountTitleSearch option||
|AccountTitle|`IEnumerable<AccountTitle>`|GetByCodes|AccountTitleSearch option|GetItems に集約可能|
...
|ApplicationControl|ApplicationControl|Get|int companyId|取得|
...
|BankAccount|`IEnumerable<BankAccount>`|Get|int[] ids|配列 id で取得|
|BankAccount|BankAccount|GetByCode|BankAccount account| CompanyId, BankCode, BranchCode, AccountTypeId, AccountNumber を 指定 **他メソッドに集約**|
|BankAccount|BankAccount|GetByBranchName|BankAccount account|CompanyId, BankCode, Branchname, AccountTypeId, AccountNumber を 指定 **他メソッドに集約**|
|BankAccount|`IEnumerable<BankAccount>`|GetItems|BankAccountSearch option|集約先|
...
|BankAccountType|`IEnumerable<BankAccountType>`|GetItemsAsync|none|メソッド名の修正 sufix async の削除|
|BankBranch|BankBranch|Get|BankBranch branch|CompanyId, BankCode, BranchCode を指定 キーで単一項目取得 **不要**|
|BankBranch|`IEnumerable<BankBranch>`|GetItems|BankBranchSearch option||
|BankBranch|int|Delete|BankBranch branch|CompanyId, BankCoce, BranchCode を指定して実施|
...
|JuridicalPersonality|`IEnumerable<JuridicalPersonality>`|GetItems|int companyId||

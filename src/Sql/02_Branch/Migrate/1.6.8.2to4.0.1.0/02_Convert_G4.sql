...
, FORMAT
, PATH
FROM [G3_Convert].[dbo].TBLEB_SETTING s
INNER JOIN [G3_Convert].[dbo].TBLKAISYA k
ON s.KAISYACD = k.KAISYACD
INNER JOIN [G3_Convert].[dbo].TBLTANTO t
ON s.KAISYACD = t.KAISYACD
AND s.TANTOCD = t.TANTOCD
AND t.RIYO = 1
...
, YOKINSYU
, KOUZANO
, MAX(GINKOMEI)  GINKOMEI
, MAX(SHITENMEI) SHITENMEI
FROM [G3_Convert].[dbo].TBLBANK
GROUP BY KAISYACD, GINKOCD, SHITENCD, YOKINSYU, KOUZANO
) b
ON e.KAISYACD = b.KAISYACD
AND e.GINKOCD = b.GINKOCD
AND e.SHITENCD = b.SHITENCD
AND e.YOKINSYU = b.YOKINSYU
AND e.KOUZANO  = b.KOUZANO
LEFT JOIN [G3_Convert].[dbo].TBLTANTO tr
ON  e.KAISYACD = tr.KAISYACD
AND e.R_ID = tr.TANTOCD
LEFT JOIN [G3_Convert].[dbo].TBLTANTO tl

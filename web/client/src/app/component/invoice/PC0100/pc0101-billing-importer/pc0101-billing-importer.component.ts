...
@Component({
  selector: 'app-pc0101-billing-importer',
  templateUrl: './pc0101-billing-importer.component.html',
  styleUrls: ['./pc0101-billing-importer.component.css']
})
export class Pc0101BillingImporterComponent extends BaseComponent implements OnInit,AfterViewInit {
          ...
          .subscribe(
            response => {
              if (response == PROCESS_RESULT_RESULT_TYPE.FAILURE) {
                ...
                this.newCustomerCount = this.importDataResult.newCustomerCreationCount;
    ...
    this.billingService.GetImportNewCustomer(transactionImportSource)
      .subscribe(response => {
        ...
        if (response != PROCESS_RESULT_RESULT_TYPE.FAILURE) {
          ...
          this.processCustomResult = this.processResultService.processAtFailure(
            this.processCustomResult, MSG_ERR.SOMETHING_ERROR.replace("{0}","得意先出力"),
            this.partsResultMessageComponent);
        }
      });
  }
  ...
  public openImporterSettingModal() {
    ...
    componentRef.instance.Title = "請求";
